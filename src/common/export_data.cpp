...
void ExportCsvResults(const std::string& strFilename, const Workspace& workspace, int num)
{
	...
	fout << "Q,max res,metab ratio,peak metab ratio,metab FWHM (PPM),metab FWHM (Hz),SNR,SNR max,SNR metab,spec noise,td noise,ref,init beta,final beta,final beta (PPM),phi0 (deg),phi1 (deg/PPM),water amp,water FWHM (Hz),water FWHM (PPM),water freq (Hz),WNR,res water amp,res water freq,baseline dev,max baseline,min baseline,baseline shape,initial residual,final residual,stopping reason" << std::endl;
    ...
    const std::vector<double>& res_water_amp_vec = workspace.GetResWaterAmp();
    std::vector<double>::const_iterator i_res_water_amp = res_water_amp_vec.begin();

    const std::vector<double>& res_water_freq_vec = workspace.GetResWaterFreq();
    std::vector<double>::const_iterator i_res_water_freq = res_water_freq_vec.begin();
...
void GetTable(std::ostringstream& fout, const Workspace& workspace, int fit_num)
{
    ...
    // water amp
    const std::vector<double>& water_amp_vec = workspace.GetNormalisationValue();
    ...
    fout << "Res. water amp.   = ";
    if ( options.GetFilenameWater() == "" || options.GetWaterWindow() == 0)
        fout << "NA\\n";
    else
        fout << (res_water_amp_vec[fit_num]/water_amp_vec[fit_num])*100.0 << "%\\n";
...
void ExportPdfResults(const std::string& strFilename, const Workspace& workspace, int fit_num)
{
    ...
    */
    ...
    savepdffit(freq_scale, all, all_extended, left, strFilename, table, title, signal_names, ext_output, pdf_stack, options.GetPPMstart(), options.GetPPMend(), options.GetGnuplotCex(), options.GetFitCol(), options.GetGnuplotXtic());
}
