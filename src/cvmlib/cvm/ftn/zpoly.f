...
CDEC$ ENDIF
      INTEGER*4   M, N, LDA, LDP
      COMPLEX*16  A(LDA*M), V(N), P(LDP*M), B(1)
      INTEGER*4   I, K, NS, NR, NQSR, MM, NRM, NWRK
      COMPLEX*16  CONE /(1.D0,0.D0)/, CZERO /(0.D0,0.D0)/
      REAL*8      Q
      CHARACTER*1 TRANS /'N'/
      INTEGER*4   NPOLY
...
15    CONTINUE
      ...
      CALL ZCOPYM (M, M, A, LDA, B, M)   ! the first of these matrices
...
20    CONTINUE
      ...
      DO 40 I = 1, NS - 1
          CALL ZAXPYM (M, M, V(I + 1), B, M, P, LDP)

          DO 50 K = 1, NR - 1
              CALL ZAXPY (MM, V(NS * K + I + 1), B, 1,
     1                                           B(K * MM + 1), 1)
50        CONTINUE

          IF (I .LE. NQSR) THEN
              CALL ZAXPY (MM, V(NS * NR + I + 1), B, 1,
     1                                           B(NR * MM + 1), 1)
          ENDIF

          CALL ZCOPY (MM, B, 1, B(NRM), 1)
          CALL ZGEMM (TRANS, TRANS, M, M, M, CONE, B(NRM), M, A, LDA,
     1                                           CZERO, B, M)
40    CONTINUE
     ...
     1                                           B, M, CONE, P, LDP)
      ...
      DO 60 K = 2, NR
          ...
          CALL ZGEMM (TRANS, TRANS, M, M, M, CONE, B(MM + 1), M,
     1                                          B(NRM), M, CZERO, B, M)
          CALL ZGEMM (TRANS, TRANS, M, M, M, CONE, B(K * MM + 1), M,
     1                                          B, M, CONE, P, LDP)
60    CONTINUE
