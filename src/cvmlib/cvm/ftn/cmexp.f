...
C     LWORK  - length of working array WORK (int)(input) - usually 64 * M
     ...
     1                  ISHEM, WORK, LWORK)   ! being referenced in 
                                               ! symmetric case only
CDEC$ IF DEFINED (FTN_EXPORTS)
CDEC$     ATTRIBUTES DLLEXPORT::CMEXP
CDEC$ ENDIF
      INTEGER*4   M, LDA, LDE
      COMPLEX*8   A(LDA*M), EA(LDE*M)
      INTEGER*4   NR, NI, NQ, J, LWORK
      LOGICAL     ISHEM
      COMPLEX*8   R(NR), WORK(LWORK)
      INTEGER*4   IR(NI)
...
CDEC$ ENDIF
      ...
      REAL*4    ONE /1./, SIX /6./
      REAL*8    DTWO /2.D0/
      REAL*8    DTWOL /0.69314718055994530941723212145818D0/
      INTEGER*4 NB, MM, NQC
      REAL*4    CINFNM
      INTEGER*4 FLOOR, CEILING
      REAL*4    TINY
      ...
      DO 10 WHILE (.TRUE.)
          TMP = EQ * ANRM
          IF (TMP * EXP (TMP) .LE. TOL .AND. NQ .GT. 1) GOTO 15
          NQ = NQ + 1
...
15    J = 0
     ...
     1             DFLOAT (CEILING (DSQRT (DFLOAT (NQC - 1)))))
      ...
      INTEGER*4   M, N
      COMPLEX*8   A(M*M), V1(N), V2(N), P1(M*M), P2(M*M), B(1), B2(1)
      COMPLEX*8   CONE /(1., 0.)/, CZERO /(0., 0.)/
