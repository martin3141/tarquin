...
#   endif
#   if (!defined (__INTEL_COMPILER) || !defined(_WIN64)) && !defined(CVM_ACML)
...
#   pragma warning(disable:4396) // visual studio 2008 has regressed!
...
public:
    ...
    basic_cmatrix& assign (const CVector& v)                                    // assigns vector
    {
        this -> _assign (v, v.incr());
        return *this;
    }

    basic_cmatrix& assign (const TC* pD)                                        // assigns a foregn array (nIncr = 1)
    {
        this -> _assign (pD, 1);
        return *this;
    }
...
protected:
    ...
    // QR decomposition
    // Case 1: economy mode, A is (m x n) and Q is (m x n) and R is (n x n)
    virtual void _qr(basic_cmatrix<TR, TC>& pmQ, basic_scmatrix<TR, TC>& pmR) const throw (cvmexception)
    {
        if (this -> mnM != pmQ.msize() || this -> mnN != pmQ.nsize() || this -> mnN != pmR.msize()) throw cvmexception (CVM_SIZESMISMATCH);
        __qre<basic_cmatrix, basic_scmatrix<TR, TC> > (*this, pmQ, pmR);
    }
    // Case 2: full mode, A is (m x n) and Q is (m x m) and R is (m x n)
    virtual void _qr(basic_scmatrix<TR, TC>& pmQ, basic_cmatrix<TR, TC>& pmR) const throw (cvmexception)
    {
        if (this -> mnM != pmQ.msize() || this -> mnM != pmR.msize() || this -> mnN != pmR.nsize()) throw cvmexception (CVM_SIZESMISMATCH);
        __qrf<basic_cmatrix, basic_scmatrix<TR, TC> > (*this, pmQ, pmR);
    }
...
public:
    ...
    basic_scmatrix& assign (const CVector& v)                                   // assigns vector
    {
        this -> _assign (v, v.incr());
        return *this;
    }

    basic_scmatrix& assign (const TC* pD)                                       // assigns foregn array (nIncr = 1)
    {
        this -> _assign (pD, 1);
        return *this;
    }
...
protected:
    ...
    // QR - "economy" mode
    virtual void _qr(basic_scmatrix<TR, TC>& pmQ, basic_scmatrix<TR, TC>& pmR) const throw (cvmexception)
    {
        if (this -> mnM != pmQ.msize() || this -> mnM != pmR.msize()) throw cvmexception (CVM_SIZESMISMATCH);
        __qre<basic_cmatrix<TR, TC>, basic_scmatrix<TR, TC> > (*this, pmQ, pmR);
    }
...
public:
    ...
    basic_scbmatrix& assign (const CVector& v)                                  // assigns vector
    {
        this -> _assign (v, v.incr());
        return *this;
    }

    basic_scbmatrix& assign (const TC* pD)                                      // assigns a foregn array (nIncr = 1)
    {
        this -> _assign (pD, 1);
        return *this;
    }
...
public:
    ...
    basic_schmatrix (TC* pD, int nMN, TR tol = basic_cvmMachSp<TR>())
    ...
    basic_schmatrix (const TR* pRe, const TR* pIm, int nMN, TR tol = basic_cvmMachSp<TR>())
    ...
    basic_schmatrix& assign (const CVector& v, TR tol = basic_cvmMachSp<TR>()) throw (cvmexception)             // assigns vector
    {
        this -> _assign (v, v.incr());
        this -> _check_hermitian(tol);
        return *this;
    ...
    basic_schmatrix& assign (const TC* pD, TR tol = basic_cvmMachSp<TR>()) throw (cvmexception)                 // assigns foregn array (nIncr = 1)
    {
        this -> _assign (pD, 1);
        this -> _check_hermitian(tol);
        return *this;
...
protected:
    ...
    void _check_hermitian (TR tol) const throw (cvmexception)
    {
        for (int j = 0; j < this -> mnN; ++j)
        {
            ...
            {
                if (i == j) {
                    if (_abs (this -> mpD[this -> mnLD * j + i].imag()) > tol)    // real numbers on main diagonal
                    {
                        throw cvmexception (CVM_MATRIXNOTHERMITIAN);
                    }
                    continue;
                }
                if (!_conjugated<TR> (this -> mpD[this -> mnLD * j + i], this -> mpD[this -> mnLD * i + j], tol))
                {
                    throw cvmexception (CVM_MATRIXNOTHERMITIAN);
                }
            }
    ...
    virtual void _set_imag_number(TR) throw (cvmexception) {throw cvmexception (CVM_METHODNOTAVAILABLE, "set_imag", "schmatrix");}
};
